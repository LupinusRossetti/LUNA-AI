...
for %%p in (3000 3001 8000 9000) do (
...
REM --------------------------------------------
REM 6) ws_config_api.py 起動（Next.js の設定読込用）
REM --------------------------------------------
echo Starting ws_config_api.py...
start "WS_CONFIG" cmd /k "cd C:\LUNA-AI\python && python ws_config_api.py"
...
start "CHARACTER_A" cmd /k "cd C:\LUNA-AI\python && set PROMPT_FILE=%PROMPT_FILE_A% && set WS_URL=ws://localhost:8000/wsAB && python characterA.py"
...
start "CHARACTER_B" cmd /k "cd C:\LUNA-AI\python && set PROMPT_FILE=%PROMPT_FILE_B% && set WS_URL=ws://localhost:8000/wsAB && python characterB.py"


REM --------------------------------------------
REM 9) AITuberKit A & B 同時起動
REM --------------------------------------------
