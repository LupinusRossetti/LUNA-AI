...
- **毎回ではなく、必要に応じて使用**
- **使用タイミング**:
- **正確性の確保**: サーチグラウンディングが実際に使用された場合のみ、`(サーチ)`を表示し、正確な情報を提供する
  - 最新情報が必要な場合
...
### 1. 長期記憶システム（Long-term Memory）

**目的**: 会話を通じてキャラクターが成長し、過去の会話内容を記憶できるようにする。全履歴をコンテキストに含めるとトークン数が肥大化するため、関連記憶のみを動的に注入する仕組みを実装する。

**要件**:
- **記憶の抽出と保存**: 会話の中でユーザーの「名前」「好み」「重要な出来事」「固有名詞」などを抽出して保存
- **関連記憶の注入（RAG的アプローチ）**: 新しいメッセージが来た際、保存された記憶の中から「今回の会話に関連するもの」だけを検索・抽出し、システムプロンプトに動的に追加
- **実装方針**: 
  - まずは `src/features/memory` ディレクトリを作成
  - ローカルのJSONファイル等で管理、または簡易的な類似度検索（キーワードマッチ）で実装
  - 大掛かりなベクターDBは使用しない（初期実装）

**実装場所**:
- `src/features/memory/` - 記憶管理モジュール
- `src/pages/api/ai/vercel.ts` - AIリクエスト前に関連記憶を検索し、システムプロンプトに注入

### 2. 回答品質向上プロンプト（Self-Reflection）

**目的**: AIの回答品質を劇的に向上させ、プロンプトのルールを絶対に守らせる。

**要件**:
- システムプロンプト（`prompts/iris.txt` 等の内容）の直後、または最も効果的な位置に `<self_reflection>` ルールを必ず適用
- プロンプト構成: キャラクター設定 + 関連記憶 + Self-Reflection + 会話履歴 の順序でコンテキストを構成

**Self-Reflectionプロンプト**:
```xml
<self_reflection>
...
**実装場所**:
- `src/pages/api/ai/vercel.ts` - システムプロンプト生成時にSelf-Reflectionプロンプトを組み込む
- `src/features/chat/handlers.ts` - システムプロンプト生成ロジックを修正

### 3. サーチグラウンディング機能の正確性向上

**目的**: サーチグラウンディング機能が実際に使用されたかどうかを正確に検出し、間違った情報を表示しないようにする。

**現状の問題**:
- XMLタグに`search="true"`が含まれていても、実際にはサーチグラウンディングが使用されていない場合がある
- サーチグラウンディングが使用されていない場合でも、`(サーチ)`が表示されてしまう
- 間違った情報が表示される

**要件**:
- サーチグラウンディングが実際に使用されたかどうかを正確に検出する仕組み
- サーチグラウンディングが使用されていない場合は、`(サーチ)`を表示しない
- サーチグラウンディングが使用された場合のみ、正確な情報を表示する

**実装場所**:
- `src/pages/api/ai/vercel.ts` - サーチグラウンディング検出ロジックの改善
- `src/features/chat/handlers.ts` - サーチグラウンディング情報の伝達ロジックの改善

### 4. 掛け合いターン数の厳守

**目的**: サーチグラウンディングを使用した掛け合いは、必ず7ターン以上になるようにする。

**現状の問題**:
- サーチグラウンディングを使用した掛け合いが7ターン未満になることがある
- システムプロンプトの指示が守られていない

**要件**:
- サーチグラウンディングを使用した掛け合いは、必ず7ターン以上になるようにシステムプロンプトを強化
- 7ターン未満の掛け合いはエラーとして扱う
