...
### 5. 2段階処理（サーチグラウンディング + 掛け合い）のテスト

#### テスト手順
1. 掛け合いモード（`NEXT_PUBLIC_DIALOGUE_MODE=true`）で起動
2. サーチグラウンディングが必要な話題で会話
   - 例: 「最新のゲーム情報を教えてサーチ」
3. 2段階処理が実行されることを確認

#### 確認すべきログ
- `[vercel.ts] 🔍 Stage 1 開始: サーチグラウンディングで情報取得中...` - Stage 1の開始
- `[vercel.ts] ✅ Stage 1 完了: 情報取得成功` - Stage 1の完了
- `[vercel.ts] 🎭 Stage 2: 掛け合いを生成中...` - Stage 2の開始
- `[vercel.ts] ✅ Stage 2 開始: 掛け合いをストリーミング中` - Stage 2のストリーミング開始

#### 期待される動作
- Stage 1でサーチグラウンディングを使用して最新情報をプレーンテキストで取得
- Stage 2で取得した情報を使用してXML形式の掛け合いを生成（7ターン以上、500文字以内）
- キャラクター名は環境変数から取得される（ハードコーディングなし）
